...
def handler(event, context):
    print("Received event for sample two: " + json.dumps(event))
    request = EvaluationRequest.from_dict(event)
    repo = request.repositories[0]
    evaluation = Evaluation(repository=repo, score=50.0, requirement_group=RequirementGroup.ENGINEERING, requirement="sample-two", status=EvaluationStatus.PASS, badge=Badge.GOLD, gh_check=False, blob="")
